...
class WCTMW_Wpml
{
	...
	{
		if(!isset($_POST['wctmw_wpml_language']) || !$this->wpml_is_active())
			return;
		
		global $sitepress;
		load_plugin_textdomain('woocommerce-product-fields-checkout', false, WCTMW_PLUGIN_LANG_PATH );
		$sitepress->switch_lang($_POST['wctmw_wpml_language'], true);
	}
	...
	public function get_all_translation_ids($post_id, $post_type = "product")
	{
		...
		//if($post_type == "product")
		{
			...
			//wctmw_var_dump($translations_result);
		}
	...
	/* function unregister_fields($fields)
	{
		if (function_exists ( 'icl_unregister_string' ) && class_exists('SitePress'))
			foreach((array)$fields as $old_field)
				{
					icl_unregister_string ( 'woocommerce-product-fields-checkout', 'wctmw_'.$old_field->cid );
					if(isset($old_field->field_options->options))
						foreach($old_field->field_options->options as $index => $extra_option)
						{
							if(isset($extra_option->label))
								icl_unregister_string ( 'woocommerce-product-fields-checkout','wctmw_'.$old_field->cid."_sublabel_".$index);
						}
					if(isset($old_field->field_options->description))
							icl_unregister_string ( 'woocommerce-product-fields-checkout', 'wctmw_'.$old_field->cid.'_description');
				}
	}*/
	function translate_single_string($id, $text, $prefix = "wctmw_label_"  )
	{
		if(!class_exists('SitePress'))
			return $text;
		
		$result = apply_filters( 'wpml_translate_single_string', $text, 'woocommerce-product-fields-checkout', 'wctmw_'.$prefix.$id, ICL_LANGUAGE_CODE  );
			
		return $result != "" ? $result : $text;
	}
	function register_single_string($id, $text, $prefix = "wctmw_label_" )
	{
		do_action( 'wpml_register_single_string', 'woocommerce-product-fields-checkout', 'wctmw_'.$prefix.$id, $text);
	}
